// Transcrypt'ed from Python, 2020-05-01 11:49:14
var math = {};
var random = {};
var settings = {};
...
export var Human =  __class__ ('Human', [Element], {
	__module__: __name__,
	get __init__ () {return __get__ (this, function (self, position, status, stationary) {
	...
	get _position_to_bounds () {return __get__ (this, function (self) {
		...
		if (self._position [0] > settings.WIDTH - self._radius) {
			self._position = tuple ([settings.WIDTH - self._radius, self._position [1]]);
			self._velocity = tuple ([-(self._velocity [0]), self._velocity [1]]);
		}
		...
		if (self._position [1] > settings.HEIGHT - self._radius) {
			self._position = tuple ([self._position [0], settings.HEIGHT - self._radius]);
			self._velocity = tuple ([self._velocity [0], -(self._velocity [1])]);
		}
	});}
});
